...
import { Heart, Settings, Share, UserCheck, UserPlus, Mail, Calendar, Shield, Play, MessageCircle, Clock, CheckCircle, X, Copy } from "lucide-react";
...
const ProfileScreen = ({ userId: propUserId }) => {
  ...
  // Extract this component or keep it in the same file
 const InviteModal = ({ user, onClose }) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Join me on ClipStream and start creating amazing content! Use my invite link to sign up.`,
          url: inviteLink
        });
      } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl w-full max-w-md shadow-2xl border border-gray-700">
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-300 mt-1">
                  Get <span className="text-yellow-400 font-bold">10 coins</span> for each friend who signs up using your link
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-800/50 rounded-lg p-4 text-center border border-gray-700">
              <p className="text-2xl font-bold text-pink-500">{user.totalInvites || 0}</p>
              <p className="text-xs text-gray-400 mt-1">Total Invites</p>
            </div>
            <div className="bg-gray-800/50 rounded-lg p-4 text-center border border-gray-700">
              <p className="text-2xl font-bold text-purple-500">{(user.totalInvites || 0) * 10}</p>
          ...
          <div>
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                value={inviteLink}
                readOnly
                className="flex-1 bg-gray-800 border border-gray-600 rounded-lg px-4 py-3 text-sm text-gray-300 focus:outline-none focus:ring-2 focus:ring-pink-500"
              />
              <button
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent('Join me on ClipStream! ' + inviteLink)}`, '_blank')}
                className="flex items-center justify-center space-x-2 bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg transition-colors"
              >
              ...
              <button
                onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent('Join me on ClipStream! ' + inviteLink)}`, '_blank')}
                className="flex items-center justify-center space-x-2 bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-lg transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-gray-800/30 rounded-lg p-4 border border-gray-700">
            ...
            <ol className="text-xs text-gray-400 space-y-1 list-decimal list-inside">
              ...
              <li>Use coins for premium features</li>
            </ol>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-gray-700 bg-gray-800/30">
          <button
            onClick={handleShare}
            className="w-full bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white py-3 rounded-lg font-medium transition-all transform hover:scale-105 flex items-center justify-center space-x-2"
          >
